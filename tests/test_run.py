...
def test_run_run(run):
    assert run() == 123
    assert (run.start_time - datetime.datetime.now(datetime.UTC)).total_seconds() < 1
    assert (run.stop_time - datetime.datetime.now(datetime.UTC)).total_seconds() < 1
...
def test_run_heartbeat_event(run):
    ...
    assert (call_kwargs["beat_time"] - datetime.datetime.now(datetime.UTC)).total_seconds() < 1
